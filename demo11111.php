		...
		if(isset($_POST["dangky"])){ //isset(): là hàm kiểm tra một biến có tồn tại hay không.
			...
			if($pass1!=$pass2 || $pass1 =="" || $first_name =="" || $last_name == "" || $phone == "" || $address== ""){
                ...
                //md5(): là hàm mã hóa một chuỗi thành 1 dãy kí tự gồm 32 ký tự.
                    mysqli_query($conn,"
                        insert into users (first_name,last_name,phone,email,password,street_address,city,state,zip_code,role)
                        values ('$first_name','$last_name','$phone','$email','$pass','$address','$city','$state','$zipcode','0')
                    ");
                    include "admin.php";
	...
	<?php
		if(isset($_POST["dangnhap"])){
			...
			$rows = mysqli_query($conn,"
				select * from users where email = '$tk' and password = '$mk' and role = '$role'
            ");
